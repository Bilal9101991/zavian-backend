...
# --- Load environment (local dev). On Render you set env vars in the dashboard. ---
load_dotenv()
...
ALLOWED_ORIGINS: List[str] = [x.strip() for x in _frontend_env.split(",") if x.strip()]
...
#   Set via env: DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASS, DB_SSLMODE
# ------------------------------------------------------------------------------
...
DB_SSLMODE = os.getenv("DB_SSLMODE", "require")  # "require" for Neon/Render
...
def ensure_aurigen_tables() -> None:
    with psycopg.connect(DB_DSN, row_factory=dict_row) as conn, conn.cursor() as cur:
        cur.execute(
            """
